...
#define PREFETCH "OFFCORE_RESPONSE_0:HWPF_L1D_AND_SWPF_LOCAL_DRAM"
...
void no_attack(const uint32_t nb_loops, const uintptr_t addr1, const uintptr_t addr2)
{
    ...
    index = rand() % 163840;
    printf("%llu, %d\n", index, (5<<(6+7+2+2))/sizeof(uint32_t));
    ...
    /* Print the result */
    printf("hits(L1): %lld, misses(L3): %lld, PREFETCH: %lld\n", values[0], values[1], values[2]);
}
...
void hammer() {
  ...
  FLUSH(vict.addr);
  no_attack((RH_THRESHOLD+1)/2, agg1.addr, agg2.addr);
  FLUSH(vict.addr);
  /* printf("vict:%xh[%d|%d|%xh|%xh] = %08Xh", vict.addr, vict.bank_group, vict.bank, vict.row,
                                            vict.column, *((int*)vict.addr));*/
}
...
int main() {
  ...
  /* Initialize the library */
    ...
    /* Find the code for the native event */
    retval = PAPI_event_name_to_code(IXCMISS, &native);
    if (retval != PAPI_OK){
        printf("event error");
        handle_error(retval);
    }
